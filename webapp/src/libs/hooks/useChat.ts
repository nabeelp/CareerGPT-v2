...
export function getFriendlyChatName(convo: ChatState): string {
    ...
    // Regex to match the CareerGPT timestamp format that is used as the default chat name.
    // The format is: 'CareerGPT @ MM/DD/YYYY, hh:mm:ss AM/PM'.
    const autoGeneratedTitleRegex =
        /CareerGPT @ [0-9]{1,2}\/[0-9]{1,2}\/[0-9]{1,4}, [0-9]{1,2}:[0-9]{1,2}:[0-9]{1,2} [A,P]M/;
    const firstUserMessage = messages.find(
        (message) => message.authorRole !== AuthorRoles.Bot && message.type === ChatMessageType.Message,
    );

    // If the chat title is the default CareerGPT timestamp, use the first user message as the title.
